...
function iniciarBuffers() {
    ...
    //... Color del pentagono
    pen1C= colorPoligono([1.0, 1.0, 0.0, 1.0], 5);
...
function triangulo(pTriangulo, pTrianguloC, pTraslacion, pAngulo, pEjeRotacion,pEjeRotacion_y, pEscala){
...
function poligono(pPoligono, pPoligonoC, pTraslacion, pAngulo,pAngulo2, pEscala){
    mat4.identity(mvMatriz);
    mat4.translate(mvMatriz, pTraslacion);
    mat4.rotate(mvMatriz, pAngulo, [1,0,0]);
    mat4.rotate(mvMatriz, pAngulo2, [0,1,0]);
    mat4.scale(mvMatriz, pEscala);
...
function poligono3D(pPoligono, pPoligonoC, pPoligonoI, pTraslacion, pAngulo,pAngulo2, pEscala){
    mat4.identity(mvMatriz);
    mat4.translate(mvMatriz, pTraslacion);
    mat4.rotate(mvMatriz, pAngulo, [1,0,0]);
    mat4.rotate(mvMatriz, pAngulo2, [0,1,0]);
...
function dibujarEscena() {
    ...
    //... dibujando pentagono
    poligono(pen1, pen1C, [0.0, 0.0, coordZ], rotarX*Math.PI/180 ,rotarY*Math.PI/180, [1.0, 1.0, 1.0]);
    poligono(pen2, pen2C, [0.0, 0.0, coordZ], rotarX*Math.PI/180 , rotarY*Math.PI/180,[1.0, 1.0, 1.0]);
    poligono3D(lPen1, lPen1C, lPen1I, [0.0, 0.0, coordZ], rotarX*Math.PI/180,rotarY*Math.PI/180, [1.0, 1.0, 1.0]);
}
...
function manejoKeys() {
    ...
    if (keyPrecionado[38]) {
        velX-= 5; // Up cursor key
    }
    if (keyPrecionado[40]) {
        velX+= 5; // Down cursor key
    }
}
var ultimoTiempo= 0;
function animacion() {
